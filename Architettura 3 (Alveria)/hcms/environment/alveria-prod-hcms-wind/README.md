...
| <a name="input_hcms_tg_cfg"></a> [hcms\_tg\_cfg](#input\_hcms\_tg\_cfg) | n/a | <pre>object({<br>    port                    = string<br>    health_check_path       = string<br>    domain_names            = list(string)<br>    collaudo_domain_names   = list(string)<br>    formazione_domain_names = optional(list(string), [])<br>    backoffice_domain_names = optional(list(string), [])<br>  })</pre> | n/a | yes |
